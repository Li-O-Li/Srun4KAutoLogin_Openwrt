...
void hmac_md5_old(unsigned char* out, unsigned char* data, int dlen, unsigned char* key, int klen)
{
	int i;
...
memBlock *hmac_md5(char* data, char* key){
    unsigned char * md5_res = (unsigned char *)malloc(16);
    hmac_md5_old(md5_res, (unsigned char *)data, strlen((const char*)data), (unsigned char *)key, strlen((const char*)key));
    memBlock *final_res = allocateMemBlock(sizeof(char), 37);

    char *hex="0123456789abcdef";
    for (int i = 0; i < 16; i++){
        *(((char *)final_res->data)+5+2*i) = hex[md5_res[i] >> 4];
        *(((char *)final_res->data)+6+2*i) = hex[md5_res[i] & 0x0f];
    }
    free(md5_res);
    char *prefix="{MD5}";
    memcpy(final_res->data, prefix, 5);
    return final_res;
}